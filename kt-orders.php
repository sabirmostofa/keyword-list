...
$table_heads = array('Date', 'User', 'Amount', 'Price','Keywords','Extension'); 
...
<div class='wrap' style="margin-top:20px">
	<table class='widefat'>
		<thead>
			<tr>
			<?php foreach($table_heads as $head)echo "<th >{$head}</th>"; ?>
			</tr>	
		</thead>
		<tbody>
			...
			<?php
				foreach($res as $single){
					...
					echo "<td>{$date}</td><td>{$user}</td> <td>{$num}</td><td>{$price}</td> <td>{$string}</td><td>$key_ext</td>";
					echo '</tr>';
					}
			?>
